...
router.post('/transform', rateLimit, hmacAuth, async (req: Request, res: Response) => {
  ...
  try {
    ...
    let map: import('../utils/redact.js').PIIMap | undefined;
    if (redact) {
    ...
    const id = (req as any).id || '-';
    console.warn(`[${id}] /transform ${status} error=${(err as any)?.name || 'Error'}`);
